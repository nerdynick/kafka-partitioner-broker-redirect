...
import org.apache.kafka.clients.producer.oos.OutOfSyncProxyRedirectPartitioner;
import org.apache.kafka.clients.producer.oos.OutOfSyncProxyRedirectStickyPartitionCache;
...
public class TestOutOfSyncProxyRedirectPartitioner {
    ...
    static {
        final Node[] nodes = new Node[]{
            ...
            new Node(4, "localhost", 9094),
            new Node(5, "localhost", 9095)
        };

        final List<PartitionInfo> goodPartitions = Arrays.asList(
            new PartitionInfo("test", 0, nodes[0], new Node[]{nodes[0], nodes[1], nodes[2]}, new Node[]{nodes[0], nodes[1], nodes[2]}),
            new PartitionInfo("test", 1, nodes[1], new Node[]{nodes[1], nodes[2], nodes[3]}, new Node[]{nodes[1], nodes[2], nodes[3]}),
            new PartitionInfo("test", 2, nodes[2], new Node[]{nodes[2], nodes[3], nodes[4]}, new Node[]{nodes[2], nodes[3], nodes[4]}),


            new PartitionInfo("test_proxy", 0, nodes[0], new Node[]{nodes[0], nodes[1], nodes[2]}, new Node[]{nodes[0], nodes[1], nodes[2]}),
            new PartitionInfo("test_proxy", 1, nodes[1], new Node[]{nodes[1], nodes[2], nodes[3]}, new Node[]{nodes[1], nodes[2], nodes[3]}),
            new PartitionInfo("test_proxy", 2, nodes[2], new Node[]{nodes[2], nodes[3], nodes[4]}, new Node[]{nodes[2], nodes[3], nodes[4]}),
            new PartitionInfo("test_proxy", 3, nodes[3], new Node[]{nodes[3], nodes[4], nodes[0]}, new Node[]{nodes[3], nodes[4], nodes[0]})
        );
        final List<PartitionInfo> badPartitions = Arrays.asList(
            ...
            new PartitionInfo("test_proxy", 1, nodes[1], new Node[]{nodes[1], nodes[2], nodes[3]}, new Node[]{nodes[1], nodes[2], nodes[3]}),
            new PartitionInfo("test_proxy", 2, nodes[2], new Node[]{nodes[2], nodes[3], nodes[4]}, new Node[]{nodes[2], nodes[3], nodes[4]}),
            new PartitionInfo("test_proxy", 3, nodes[3], new Node[]{nodes[3], nodes[4], nodes[0]}, new Node[]{nodes[3], nodes[4]})
        );
    ...
    public void testGoodTopic() throws InterruptedException, ExecutionException{
        ...
        configs.put(OutOfSyncProxyRedirectStickyPartitionCache.Config.PROXY_TOPIC_CONFIG, "test_proxy");

        Partitioner p = new OutOfSyncProxyRedirectPartitioner();
        p.configure(configs);
        
        final Producer<byte[], byte[]> producer = new MockProducer<>(GoodCluster, true, p, new ByteArraySerializer(), new ByteArraySerializer());
        ...
        while(count < 10000){
            ...
            assertTrue(m.partition() >= 0 && m.partition() <=4);
        }
    ...
    public void testBadTopic() throws InterruptedException, ExecutionException{
        ...
        final Producer<byte[], byte[]> producer = new MockProducer<>(BadCluster, true, p, new ByteArraySerializer(), new ByteArraySerializer());
        ...
        while(count < 10000){
            ...
            assertNotNull(m.partition());
            assertTrue(m.partition() >= 1 && m.partition() <=4, "Partition was an invalid one");
        }
            ...
            Thread t = new Thread(new Runnable(){
                ...
                public void run() {
                    for(int records = 0; records < 100; records++){
                        ...
                        assertNotNull(m.partition());
                        assertTrue(m.partition() >= 1 && m.partition() <=4, "Partition was an invalid one: "+ m.partition());
                    }
                }
            });
